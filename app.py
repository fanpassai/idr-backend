"""
IDR Scanner API
Flask REST API — production build for Railway deployment.
"""
...
@app.route('/api/receipt/<receipt_id>/summary', methods=['GET'])
def get_receipt_summary(receipt_id: str):
    receipt = RECEIPT_STORE.get(receipt_id.upper())
    if not receipt:
        return _error(f"Receipt {receipt_id} not found.", 404)
    summary = format_receipt_summary(receipt)
    return app.response_class(response=summary, status=200, mimetype='text/plain')
...
def registry(domain: str):
    ...
    if status_val == 'pass' and scan.get('critical_count', 1) == 0:
        reg_status = "active"
    else:
        reg_status = "monitoring"
    return jsonify({
        "domain": domain,
...
def activate():
    """
    Activation endpoint — receives email + store URL,
    triggers scan, returns receipt.
    """
    body = request.get_json(silent=True)
    if not body:
